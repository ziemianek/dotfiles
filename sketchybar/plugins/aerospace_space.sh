...
if [ "$focused" = "$sid" ]; then
  # Animate underline
  sketchybar --animate sin 12 --set "space_underline.$sid" background.color="$ACTIVE_UNDERLINE"

  # Dark background pill behind label
  sketchybar --animate sin 12 --set "space.$sid" \
  ...
  # Hide underline
  sketchybar --animate sin 12 --set "space_underline.$sid" background.color="$INACTIVE_UNDERLINE"
